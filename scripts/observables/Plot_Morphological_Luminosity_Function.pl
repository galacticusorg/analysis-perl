...
my $data    = $xml->XMLin($ENV{'GALACTICUS_DATA_PATH'}."/static/observations/luminosityFunctions/Morphological_Luminosity_Functions_2MASS_Devereux_2009.xml");
...
system "rm -f ".$outputFile."; cd ".$outputDir."; pdfmerge ".join(" ",@leafFiles)." tmp.pdf; cd -; mv ".$outputDir."/tmp.pdf ".$outputFile." > /dev/null 2>&1";
