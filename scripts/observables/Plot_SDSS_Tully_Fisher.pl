...
my $data    = $xml->XMLin($ENV{'GALACTICUS_DATA_PATH'}."/static/observations/tullyFisherRelation/Tully_Fisher_SDSS_Pizagno_2007.xml");
my $columns = $data->{'tullyFisher'}->{'columns'};
my $x       = pdl @{$columns->{'magnitude'}->{'data'}};
$x          = $x-5.0*log10($columns->{'magnitude'}->{'hubble'}/$dataSet->{'parameters'}->{'cosmologyParameters'}->{'HubbleConstant'}->{'value'});
