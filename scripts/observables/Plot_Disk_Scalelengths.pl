...
my $data = $xml->XMLin($ENV{'GALACTICUS_DATA_PATH'}."/static/observations/galaxySizes/Disk_Sizes_Dejong_2000.xml");
my $i = -1;
foreach my $dataSet ( @{$data->{'sizeDistribution'}} ) {
    ...
    $title =~ s/([\-\d]+\s*<\s*)M(\s*<\s*[\-\d]+)/\$$1M_\\mathrm{I,0}$2\$/;
    print $gnuPlot "set title offset 0,-1 '".$title."'\n";
    print $gnuPlot "set xlabel '\$r_\\mathrm{disk}\$ [kpc]'\n";
    print $gnuPlot "set ylabel '\$\\mathrm{d}^2n/\\mathrm{d}\\log_{10}r_\\mathrm{disk}/\\mathrm{d}M_\\mathrm{I,0}\\,\\,[\\hbox{Mpc}^{-3} \\hbox{mag}^{-1}]\$'\n";
