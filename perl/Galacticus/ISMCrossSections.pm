...
sub Cross_Sections {
	...
	unless ( defined($mm83Energy) ) {
	    ...
	    my $absorptionFit = $xml->XMLin($ENV{'GALACTICUS_DATA_PATH'}."/static/atomic/Interstellar_Photoelectric_Absorption_Morrison_McCammon_1983.xml");
	...
	unless ( -e $ENV{'GALACTICUS_DATA_PATH'}."/static/atomic/Interstellar_Absorption_Wilms_2000.hdf5" ) {
	    unless ( -e $ENV{'GALACTICUS_EXEC_PATH'}."/XRay_Absorption_ISM_Wilms2000.exe" ) {
		system("cd ".$ENV{'GALACTICUS_EXEC_PATH'}."; make XRay_Absorption_ISM_Wilms2000.exe");
		die("Unable to build XRay_Absorption_ISM_Wilms2000.exe")
		    unless ( -e $ENV{'GALACTICUS_EXEC_PATH'}."/XRay_Absorption_ISM_Wilms2000.exe" );
		# Build the absorption table.
		system($ENV{'GALACTICUS_EXEC_PATH'}."/XRay_Absorption_ISM_Wilms2000.exe");
	    }
	}
	# Read the data file.
	unless ( defined($w00Energy) ) {
	    my $absorptionFile = new PDL::IO::HDF5($ENV{'GALACTICUS_DATA_PATH'}."/static/atomic/Interstellar_Absorption_Wilms_2000.hdf5");
