...
sub Get_Flux {
	...
	unless ( defined(first { $_ eq "SED" } @datasetsList) && $recomputeSEDs == 0 ) {
	    ...
	    open(iHndl,$ENV{'GALACTICUS_DATA_PATH'}."/static/grasil/grasilBaseParameters.txt");
...
sub Process_Through_Grasil {
	...
	my $grasilRun = "cd `dirname ".$grasilJob->{'grasilFilesRoot'}."`; ".$ENV{'GALACTICUS_EXEC_PATH'}."/aux/Grasil/grasil `basename ".$grasilJob->{'grasilFilesRoot'}."`";
	my %job =
	    (
...
sub grasilGet {
    ...
    my $grasilPath = $ENV{'GALACTICUS_EXEC_PATH'}."/aux/Grasil/";
